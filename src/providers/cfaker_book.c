...
static const struct cfaker_book get_cfaker_book() {
...
const char* cfaker_book_title() {
    ...
    return book.titles[cfaker_random_int(0, book.title_count - 1)];
}

const char* cfaker_book_author() {
    const struct cfaker_book book = get_cfaker_book();
    return book.authors[cfaker_random_int(0, book.author_count - 1)];
}

const char* cfaker_book_genre() {
    const struct cfaker_book book = get_cfaker_book();
    return book.genres[cfaker_random_int(0, book.genre_count - 1)];
}

const char* cfaker_book_publisher() {
    const struct cfaker_book book = get_cfaker_book();
    return book.publishers[cfaker_random_int(0, book.publisher_count - 1)];
}

const char* cfaker_book_format() {
    const struct cfaker_book book = get_cfaker_book();
    return book.formats[cfaker_random_int(0, book.format_count - 1)];
}

const char* cfaker_book_series() {
    const struct cfaker_book book = get_cfaker_book();
    return book.series[cfaker_random_int(0, book.series_count - 1)];
}
