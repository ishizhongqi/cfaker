...
static const struct cfaker_company get_cfaker_company() {
...
const char* cfaker_company_prefix() {
    ...
    return company.prefixes[cfaker_random_int(0, company.prefix_count - 1)];
}

const char* cfaker_company_suffix() {
    const struct cfaker_company company = get_cfaker_company();
    return company.suffixes[cfaker_random_int(0, company.suffix_count - 1)];
}
