...
const char* cfaker_phone_code(bool use_locale) {
    if (use_locale) {
        ...
        if (phone.country_code_count > 0) {
            return phone.country_codes[cfaker_random_uint(0, phone.country_code_count - 1)];
        }
        return "+1"; /* Fallback */
    }
    return global_country_codes[cfaker_random_uint(0, global_country_code_count - 1)];
}

const char* cfaker_phone_number() {
    const struct cfaker_phone phone = get_cfaker_phone();
    const char* format = phone.formats[cfaker_random_uint(0, phone.format_count - 1)];
    const char* number;

    /* Handle mobile prefixes (e.g., zh_CN) */
    if (phone.mobile_prefix_count > 0) {
        const char* prefix = phone.mobile_prefixes[cfaker_random_uint(0, phone.mobile_prefix_count - 1)];
